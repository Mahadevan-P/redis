...
public class ChatMessageServiceImpl implements ChatMessageService {
	...
	public ChatMessage getChatMessageById(long id) {
		...
		Optional<ChatMessage> chatmessageOp = chatmessageRepository.findById(id);

		if (chatmessageOp != null) {
			logger.debug(" << ChatMessageService : Exiting getChatMessage");
			return chatmessageOp.get();
		} else {
			logger.debug(" << ChatMessageService : No Such ChatMessage Exists : Exiting getChatMessage");
			return null;
		}
